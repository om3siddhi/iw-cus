  ...
  try {
    ...
    if (req.user) {
      if(!req.user.emailVerified){
        const currUser = req.user;
            res.render("emailverification", {currUser});
      }else{
        const currUser = req.user;
      ...
      res.render("webdetails", { error: null, blog });
    }
  } catch (error) {
    console.log(error);
  }
...
router.post('/email-otp-verification', async (req, res) => {
  const otp2 = parseInt(req.body.otp2); 

  if (!req.user) {
    return res.status(401).send("User not authenticated.");
  }

  const otp1 = req.user.otp;
  ...
  if (otp1 === otp2) {
    ...
    return res.send("OTP not verified. Please try again.");
  }
});


router.post("/email-modification", async (req, res) => {
...
router.get("/chats", async (req, res) => {
  try {
    ...
    const currUser = req.user;
    const chats = await getChatList(currUser.id);
...
router.get("/p-policy", async(req, res)=>{
  try {
    res.render("p-policy");
  } catch (error) {
    res
...
const addSampleSuccessStories = async () => {
  try {
    const stories = [
      {
        photo: "https://res.cloudinary.com/dhfjzpiuo/image/upload/v1744228868/success_stories/yppvaqzquxl1wywsckpw.jpg",
        coupleName: "Ayushi & Kanak",
        comment: "Thanks to IndianWife.com, we found each other and instantly connected. Eternally grateful!"
      },
      {
        photo: "https://res.cloudinary.com/dhfjzpiuo/image/upload/v1746018823/salonishivank_dc1ewu.jpg",
        coupleName: "Saloni & Shivank",
        comment: "This platform made it so easy to meet someone who truly matches my values. Thank you for bringing us together!"
      },
      {
        photo: "https://res.cloudinary.com/dhfjzpiuo/image/upload/v1744172099/rupaliAman_ls22i7.jpg",
        coupleName: "Rupali & Aman",
        comment: "We just wanted to say thank you for helping us find love in the most genuine way. Forever thankful!"
      }
    ];
    ...
    console.error("Error inserting success stories:", error.message);
  }
};
